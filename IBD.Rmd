...
VII.  Gene Scores
XI.   Plotting Integrated Ranking
...
DEG.chr.1.2.12 <- DEG.all %>% 
  filter(Chr %in% c(1, 2, 12)) %>% 
...
#1. Intestine
score.candidates(results.int.dir, candidate.genes = DEG.chr.1.2.12[["EntrezID"]])

#2. Hemolymphoid
score.candidates(results.hemo.dir, candidate.genes = DEG.all$EntrezID)
```
...
#1. Intestine
int.final.table <- merge.gene.svm.fp(results.hipp.dir, FEHS_allgenes, gene.column.name = "GENE")

#2. Hemolymphoid
hemo.final.table <- merge.gene.svm.fp(results.temp.dir, FEHS_allgenes, gene.column.name = "GENE")
```
...
## III. Apply the trained models to DEG candidates (from cell differential expression data)
Use SVM to classify each gene, where each gene recieves a score from each SVM indicating whether the model identifies
it at part of the gene list (a positive score) or not part of the gene list (a negative score).

Score.candidates function writes out all values to two csv tables (Candidate.Gene.SVM.Scores.csv and Candidate.Gene.FP.Rates.csv). 
(the function also plots boxplots of the distributions of the SVM scores and the FP rates for each candidate gene in two jpgs (Candidate.Gene.SVM.Scores.jpg, and Candidate.Gene.FP.Rates.jpg). These plots are ordered by mean value.

The genes that are on the left are classified confidently by the SVM as being related to the IBD gene list. The distributions of their SVM scores do not cross 0.
The genes on the right hand side are classified confidently by the SVM as not being related to the IBD gene list. 
The genes in between are less confidently classified across the SVM trials. 

```{r score candidates}
#change - want to score ALL GENES from chr 1 and 2, but will only plot significant. but will use the number of genes within the box to determine ranking instead of final score

#CELL 1
#SWITCH - using DEGapc (etc.) not SIG now
score.candidates(results.dir, candidate.genes = as.character(DEGapc[["Entrez Gene ID"]]))
#include_graphics(normalizePath(paste0(module.dir[1], "/Candidate.Gene.SVM.Scores.jpg")))
#some get lost

#CELL 2
score.candidates(results2.dir, candidate.genes = as.character(DEGbcell[["Entrez Gene ID"]]))

#CELL 3
score.candidates(results3.dir, candidate.genes = as.character(DEGcd4[["Entrez Gene ID"]]))

#CELL 4
score.candidates(results4.dir, candidate.genes = as.character(DEGcd8[["Entrez Gene ID"]]))

#CELL 5
score.candidates(results5.dir, candidate.genes = as.character(DEGtreg[["Entrez Gene ID"]]))
```




## IV. Merge SVM with expression data (use this instead of the two functions)

We need to merge the SVM scores and FP rates with expression data for each gene. 

The original merge.svm.gene.info function uses the downloaded gene information from biomart to assign gene names and positions to each of the entrez gene IDs with an SVM score. 
It writes out a table for each module with information about each gene, its name, genomic coordinates, and SVM score and FP rate for each module (Candidate.Gene.Results.csv). 
It also plots the gene names on two axes. The x axis is the genomic position, and the y axis is the SVM score (Candidate.Gene.SVM.Results.jpg).

```{r merge SVM results with DEG}
#first have to create a column called entrezgene in each of the DEG tables because the function merge.svm.gene.info requires this column name, but i didn't want to change the original entrez id column name because we used that previously for the SVM scores
DEGapc$entrezgene <- as.character(DEGapc$`Entrez Gene ID`)
DEGbcell$entrezgene <- as.character(DEGbcell$`Entrez Gene ID`)
DEGcd4$entrezgene <- as.character(DEGcd4$`Entrez Gene ID`)
DEGcd8$entrezgene <- as.character(DEGcd8$`Entrez Gene ID`)
DEGtreg$entrezgene <- as.character(DEGtreg$`Entrez Gene ID`)

##error in using merge.svm.gene.info function, so take out the needed parts and then have to separate into five different cells (and rework to fit in the type of thing that the DEG tables are)
#also need entire first section each time because have five different results folders for each cell (because of reasons above)

#CELL 1 MERGE
#merge.svm.gene.info(results.dir, DEGapc)
...
module.dir.info <- get.module.dir(results.dir, dir.table = TRUE)
  ...
  names(all.results) <- dir.table[,2]
    ...
    svm.csv.file <- paste0(module.dir, "/Candidate.Gene.SVM.Scores.csv")
    fp.csv.file <- paste0(module.dir, "/Candidate.Gene.FP.Rates.csv")
    results.file <- paste0(module.dir, "/Candidate.Gene.Results.csv")
    plot.file <- paste0(module.dir, "/Candidate.Gene.SVM.Results.jpg")
  ...
  #MKL now using gene.info.table in the original, but had to change to the actual input that would have gone in the function (which is DEGapc for cell 1 etc.)  
    # MKL had to use as.character(DEGapc[["entrezgene"]]) to make it the same type of thing as gene.infor.table[,"entrezgene"]
    common.genes <- intersect(as.character(DEGapc[["entrezgene"]]), gene.ids)
    gene.locale.table <- match(common.genes, as.character(DEGapc[["entrezgene"]]))
    ...
    final.table <- cbind(data.frame(DEGapc)[gene.locale.table,], mean.svm[gene.locale.svm], mean.fp[gene.locale.svm])
    #MKL had to change columns to 59 and 60
    colnames(final.table)[c(74,75)] <- c("Mean.SVM.Score", "Mean.FP.Rate")
    
    write.table(final.table, results.file, sep = ",", quote = FALSE, row.names = FALSE)
    
##use final.table for cell 1




#CELL 2
#merge.svm.gene.info(results2.dir, DEGbcell)


module.dir.info <- get.module.dir(results2.dir, dir.table = TRUE)
  module.dir <- module.dir.info$module.dir
  dir.table <- module.dir.info$dir.table
  ...
  names(all.results) <- dir.table[,2]
    ...
    #MKL now using gene.info.table in the original, but had to change to the actual input that would have gone in the function (which is DEGapc   for cell 1 etc.)  
    # MKL had to use as.character(DEGapc[["entrezgene"]]) to make it the same type of thing as gene.infor.table[,"entrezgene"]
    common.genes2 <- intersect(as.character(DEGbcell[["entrezgene"]]), gene.ids)
    gene.locale.table2 <- match(common.genes2, as.character(DEGbcell[["entrezgene"]]))
    # head(cbind(gene.ids, gene.info.table[gene.locale.table,]))
    gene.locale.svm2 <- match(common.genes2, gene.ids)
    # head(cbind(common.genes, gene.ids[gene.locale.svm]))
    
    final.table2 <- cbind(data.frame(DEGbcell)[gene.locale.table2,], mean.svm[gene.locale.svm2], mean.fp[gene.locale.svm2])
    #MKL had to change columns to 59 and 60
    colnames(final.table2)[c(74,75)] <- c("Mean.SVM.Score", "Mean.FP.Rate")
    
    write.table(final.table2, results.file, sep = ",", quote = FALSE, row.names = FALSE)

    
    
#CELL 3
#merge.svm.gene.info(results3.dir, DEGcd4)

module.dir.info <- get.module.dir(results3.dir, dir.table = TRUE)
  module.dir <- module.dir.info$module.dir
  dir.table <- module.dir.info$dir.table
  ...
  names(all.results) <- dir.table[,2]
   ...
   #MKL now using gene.info.table in the original, but had to change to the actual input that would have gone in the function (which is DEGapc    for cell 1 etc.)  
    # MKL had to use as.character(DEGapc[["entrezgene"]]) to make it the same type of thing as gene.infor.table[,"entrezgene"]
    common.genes3 <- intersect(as.character(DEGcd4[["entrezgene"]]), gene.ids)
    gene.locale.table3 <- match(common.genes3, as.character(DEGcd4[["entrezgene"]]))
    # head(cbind(gene.ids, gene.info.table[gene.locale.table,]))
    gene.locale.svm3 <- match(common.genes3, gene.ids)
    # head(cbind(common.genes, gene.ids[gene.locale.svm]))
    
    final.table3 <- cbind(data.frame(DEGcd4)[gene.locale.table3,], mean.svm[gene.locale.svm3], mean.fp[gene.locale.svm3])
    #MKL had to change columns to 59 and 60
    colnames(final.table3)[c(74,75)] <- c("Mean.SVM.Score", "Mean.FP.Rate")
    
    write.table(final.table3, results.file, sep = ",", quote = FALSE, row.names = FALSE)
    
    
    
#CELL 4
#merge.svm.gene.info(results4.dir, DEGcd8)
    
module.dir.info <- get.module.dir(results4.dir, dir.table = TRUE)
  module.dir <- module.dir.info$module.dir
  dir.table <- module.dir.info$dir.table
  ...
  #MKL now using gene.info.table in the original, but had to change to the actual input that would have gone in the function (which is DEGapc for cell 1 etc.)  
    # MKL had to use as.character(DEGapc[["entrezgene"]]) to make it the same type of thing as gene.infor.table[,"entrezgene"]
    common.genes4 <- intersect(as.character(DEGcd8[["entrezgene"]]), gene.ids)
    gene.locale.table4 <- match(common.genes4, as.character(DEGcd8[["entrezgene"]]))
    # head(cbind(gene.ids, gene.info.table[gene.locale.table,]))
    gene.locale.svm4 <- match(common.genes4, gene.ids)
    # head(cbind(common.genes, gene.ids[gene.locale.svm]))
    
    final.table4 <- cbind(data.frame(DEGcd8)[gene.locale.table4,], mean.svm[gene.locale.svm4], mean.fp[gene.locale.svm4])
    #MKL had to change columns to 59 and 60
    colnames(final.table4)[c(74,75)] <- c("Mean.SVM.Score", "Mean.FP.Rate")
    
    write.table(final.table4, results.file, sep = ",", quote = FALSE, row.names = FALSE)

#CELL 5
#merge.svm.gene.info(results5.dir, DEGtreg)

module.dir.info <- get.module.dir(results5.dir, dir.table = TRUE)
  module.dir <- module.dir.info$module.dir
  dir.table <- module.dir.info$dir.table
  ...
  #MKL now using gene.info.table in the original, but had to change to the actual input that would have gone in the function (which is DEGapc for cell 1 etc.)  
    # MKL had to use as.character(DEGapc[["entrezgene"]]) to make it the same type of thing as gene.infor.table[,"entrezgene"]
    common.genes5 <- intersect(as.character(DEGtreg[["entrezgene"]]), gene.ids)
    gene.locale.table5 <- match(common.genes5, as.character(DEGtreg[["entrezgene"]]))
    # head(cbind(gene.ids, gene.info.table[gene.locale.table,]))
    gene.locale.svm5 <- match(common.genes5, gene.ids)
    # head(cbind(common.genes, gene.ids[gene.locale.svm]))
    
    final.table5 <- cbind(data.frame(DEGtreg)[gene.locale.table5,], mean.svm[gene.locale.svm5], mean.fp[gene.locale.svm5])
    #MKL had to change columns to 59 and 60
    colnames(final.table5)[c(74,75)] <- c("Mean.SVM.Score", "Mean.FP.Rate")
    
    write.table(final.table5, results.file, sep = ",", quote = FALSE, row.names = FALSE)
